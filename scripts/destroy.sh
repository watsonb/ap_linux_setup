...
#
# Runs the Ansible destroy.yml playbook
#
...
while getopts ":hi:e:k:" opt; do
  case ${opt} in
    h )
      printf "Usage:"
      printf "  destroy -h                   Display this help message."
      printf "  destroy -i <inventory> -e <host/group>      Destroys windows machines in the ansible <inventory> identifed by <host/group>."
      exit 0;;
    i )
      inventory=$OPTARG;;
    e )
...
printf "\nRunning the destroy.yml playbook against $inventory\n"
ansible-playbook destroy.yml -i $inventory -e "variable_host=$host_group"  --private-key=$key
